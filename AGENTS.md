Harika bir güncelleme. Projenin veritabanı bütünlüğünü korumak ve test süreçlerini sağlama almak için bu kurallar kritik. Belirttiğin maddeleri (veritabanı değişikliklerinin `supabase_schema.sql` dosyasına yansıtılması ve Playwright testlerinin lokal ortamda çalıştırılması kuralını) yönergenin ciddiyetine ve diline uygun olarak ekledim.

İşte güncellenmiş **AI Agent Instructions** dosyası:
...
| Database & Auth | [Supabase](https://supabase.com/) (`supabase-js` + `@supabase/ssr`) | 2.x / 0.12 |
...
* 🎭 **Local Playwright Execution:** When developing, testing, or debugging workflows locally, **always spin up and run Playwright in the local environment** to execute End-to-End (E2E) tests. Ensure the local dev server is active and verified via Playwright UI or headless mode before concluding that a feature works as intended.
...
* For every database schema changes, prefer updating `supabase_schema.sql` and documenting the migration steps.
